        ...
        @foreach($products as $product)
            <div class="w-full overflow-hidden h-auto bg-white dark:bg-slate-800 shadow rounded hover:shadow-lg transition-all" wire:key='{{ $product->id }}'>
                ...
                <div class="h-auto bg-white p-2 dark:text-white dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <livewire:use-ia  :type="$product->type" :weight="$product->weight" />
                    ...
                    <div wire:loading.remove.delay.long>
                        ...
                        switch ($product->type) {
                            case '1':
                            echo '<p class="font-normal text-sm dark:text-white"><b>Tipo:</b> Ouro</p>';
                            echo '<p class="font-normal text-sm dark:text-white"><b>Preço sugerido:</b> R$ ' . number_format((($prices->price_gold * $product->weight) * Auth::user()->multiplier ?? 2), 2, ',', '.') . '</p>';
                            break;
                            case '2':
                            echo '<p class="font-normal text-sm dark:text-white"><b>Tipo:</b> Prata</p>';
                            echo '<p class="font-normal text-sm dark:text-white"><b>Preço sugerido:</b> R$ ' . number_format((($prices->price_silver * $product->weight) * Auth::user()->multiplier ?? 2), 2, ',', '.') . '</p>';
                            break;
                            default:
                            echo '<p class="font-normal text-sm dark:text-white"><b>Nenhum definido</b>';
                                break;
                            }
                            @endphp
                    </div>
