<div>
    <div class="w-full flex flex-col lg:flex-row md:flex-row justify-between items-center p-3 mt-3">
        <div class="w-full lg:w-1/4 md:w-1/4 sm:lg:w-2/4 pb-0"><x-search /></div>
        ...
        @foreach($categories as $category)
            <div class="w-full overflow-hidden h-auto bg-white dark:bg-slate-800 p-2 flex justify-between items-center shadow rounded">
                <div>
                    ...
                    <p class="text-slate-950 dark:text-white italic text-sm"> 
                ...
                <div>
                    ...
                    <button type="button" wire:click="confirm('{{ $category->id }}')" class="bg-white dark:bg-slate-600 px-3 py-2 rounded text-red-500 hover:text-red-700 transition ease-in-out duration-150 shadow"><i class="fa-solid fa-trash"></i></button>
                </div>
            </div>
        @endforeach
    </div>
